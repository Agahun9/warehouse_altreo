...
class MediaMarktStorageRepository
{
    /** @var Database */
    private $database;
    ...
    public function ensureSchema(): void
    {
        ...
        $this->database->query(
            "CREATE TABLE IF NOT EXISTS mediamarkt_accounts (\n"
            ...
            . "PRIMARY KEY (id),\n"
            . "UNIQUE KEY ux_mediamarkt_accounts_slug (slug),\n"
            . "KEY idx_mediamarkt_accounts_active (is_active)\n"
            . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
        );

        $this->database->query(
            "CREATE TABLE IF NOT EXISTS mediamarkt_offers (\n"
            . "id INT UNSIGNED NOT NULL AUTO_INCREMENT,\n"
            ...
            . "updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,\n"
            . "PRIMARY KEY (id),\n"
            . "UNIQUE KEY ux_mediamarkt_offer_account_offer (account_id, offer_id),\n"
            . "KEY idx_mediamarkt_offers_account_id (account_id),\n"
            . "KEY idx_mediamarkt_offers_shop_sku (shop_sku),\n"
            . "KEY idx_mediamarkt_offers_product_sku (product_sku),\n"
            . "KEY idx_mediamarkt_offers_state_code (state_code),\n"
            . "KEY idx_mediamarkt_offers_active (active),\n"
            . "KEY idx_mediamarkt_offers_synced (last_synced_at)\n"
            . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
        );

        $this->database->query(
            "CREATE TABLE IF NOT EXISTS mediamarkt_offer_change_queue (\n"
            . "id INT UNSIGNED NOT NULL AUTO_INCREMENT,\n"
            . "account_id INT UNSIGNED NOT NULL,\n"
            ...
            . "updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,\n"
            . "PRIMARY KEY (id),\n"
            . "KEY idx_mediamarkt_offer_change_queue_status (status, available_at),\n"
            . "KEY idx_mediamarkt_offer_change_queue_offer_row (offer_row_id),\n"
            . "KEY idx_mediamarkt_offer_change_queue_account (account_id)\n"
            . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
        );

        $this->database->query(
            "CREATE TABLE IF NOT EXISTS mediamarkt_cache (\n"
            ...
            . "KEY idx_mediamarkt_cache_expires_at (expires_at)\n"
            . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
        );
        ...
        // ~10 minutes time-boxes each account's catalog sync (see MediaMarktService::syncAccount)
        ...
        $this->ensureColumnExists('mediamarkt_accounts', 'sync_offset', 'INT UNSIGNED NOT NULL DEFAULT 0');
        ...
        $this->ensureIndexExists(
            'mediamarkt_offer_change_queue',
            'idx_mediamarkt_offer_change_queue_offer_operation',
    ...
    public function fixSkuCollation(string $collation = 'utf8mb4_unicode_ci'): array
    {
        return array_values(array_filter(array(
            $this->ensureColumnCollation('mediamarkt_offers', 'shop_sku', 'VARCHAR(190) DEFAULT NULL', $collation),
            $this->ensureColumnCollation('mediamarkt_offers', 'product_sku', 'VARCHAR(190) DEFAULT NULL', $collation),
        )));
    }
    ...
    {
        $this->database->delete('mediamarkt_cache', 'expires_at < :now', array('now' => date('Y-m-d H:i:s')));
    }

    public function allAccounts(): array
    {
        return $this->database->fetchAll('SELECT * FROM mediamarkt_accounts ORDER BY is_active DESC, name ASC, id ASC');
    }

    public function activeAccounts(): array
    {
        return $this->database->fetchAll('SELECT * FROM mediamarkt_accounts WHERE is_active = 1 ORDER BY name ASC, id ASC');
    }
    ...
    public function accountsDueForSync(): array
    {
        return $this->database->fetchAll(
            'SELECT * FROM mediamarkt_accounts WHERE is_active = 1'
    ...
    {
        return $this->database->fetch('SELECT * FROM mediamarkt_accounts WHERE id = :id LIMIT 1', array('id' => $id));
    }

    public function findAccountBySlug(string $slug)
    {
        return $this->database->fetch('SELECT * FROM mediamarkt_accounts WHERE slug = :slug LIMIT 1', array('slug' => $slug));
    }

    public function saveAccount(array $data, ?int $accountId = null): int
    {
        if ($accountId !== null && $accountId > 0) {
            $this->database->update('mediamarkt_accounts', $data, 'id = :id', array('id' => $accountId));
            return $accountId;
        }

        return (int) $this->database->insert('mediamarkt_accounts', $data);
    }

    public function markAccountSyncSuccess(int $accountId): void
    {
        $this->database->update('mediamarkt_accounts', array(
    ...
    /**
     * Returns the current local values for a page downloaded from MediaMarkt. This lets the
    ...
    public function upsertOffers(array $rows): int
    {
        ...
        $this->database->query(
            'INSERT INTO mediamarkt_offers (' . implode(', ', $columns) . ') VALUES ' . implode(', ', $valueGroups)
    ...
    public function countOffers(array $filters = array()): int
    {
        ...
        $sql = 'SELECT COUNT(*) FROM mediamarkt_offers offers'
    ...
    public function listOffers(array $filters, int $page, int $perPage, string $sortBy = 'synced', string $sortDir = 'desc'): array
    {
        ...
        $idSql = 'SELECT offers.id FROM mediamarkt_offers offers'
            . ' INNER JOIN mediamarkt_accounts accounts ON accounts.id = offers.account_id';
    ...
    public function listOffersByIds(array $offerRowIds): array
    {
        ...
        $rows = $this->database->fetchAll(
            'SELECT offers.id FROM mediamarkt_offers offers WHERE offers.id IN (' . implode(', ', $placeholders) . ') ORDER BY offers.id DESC',
            $params
        );
    ...
    {
        $offerRowIds = array_values(array_unique(array_filter(array_map('intval', $offerRowIds))));
        $select = 'SELECT offers.id, offers.account_id, offers.shop_sku FROM mediamarkt_offers offers'
            . ' INNER JOIN mediamarkt_accounts accounts ON accounts.id = offers.account_id';
    ...
    public function updateLeadtimeToShip(array $offerRowIds, int $days): void
    {
        foreach (array_chunk(array_values(array_map('intval', $offerRowIds)), 1000) as $chunk) {
            ...
            $this->database->execute(
                'UPDATE mediamarkt_offers SET leadtime_to_ship = :leadtime_days WHERE id IN (' . implode(', ', $placeholders) . ')',
                $params
            );
        }
    }
    ...
    public function queueCounts(): array
    {
        $rows = $this->database->fetchAll(
            ...
            . ' FROM mediamarkt_offer_change_queue queue_latest'
            . ' INNER JOIN ('
            . '   SELECT offer_row_id, MAX(id) AS latest_id'
            . '   FROM mediamarkt_offer_change_queue'
            . '   GROUP BY offer_row_id'
            . ' ) queue_max ON queue_max.latest_id = queue_latest.id'
            . ' INNER JOIN mediamarkt_offers offers ON offers.id = queue_latest.offer_row_id'
            . ' INNER JOIN mediamarkt_accounts accounts ON accounts.id = offers.account_id'
    ...
    public function warehouseUpdateDiagnostics(): array
    {
        $rows = $this->database->fetchAll(
            ...
            . ' FROM mediamarkt_offers offers'
            . ' INNER JOIN mediamarkt_accounts accounts ON accounts.id = offers.account_id AND accounts.is_active = 1'
        ...
        $unresolvedRows = $this->database->fetchAll(
            'SELECT operation, status, COUNT(*) AS total FROM mediamarkt_offer_change_queue'
    ...
    public function enqueueOfferChanges(array $targets, string $operation, array $payload, ?string $availableAt = null): int
    {
        ...
        foreach ($targets as $target) {
            ...
            $this->database->insert('mediamarkt_offer_change_queue', array(
    ...
    public function unresolvedQueueCount(string $operation, ?int $accountId = null): int
    {
        ...
        $sql = 'SELECT COUNT(*) FROM mediamarkt_offer_change_queue'
    ...
    public function cancelUnresolvedWarehouseUpdates(): int
    {
        return $this->database->update(
            ...
            array(
                'status' => 'cancelled',
                'error_message' => 'Anulowano: maintenance MediaMarkt dziala w trybie tylko do odczytu.',
    ...
    public function offersDueForOperation(array $filters, string $operation, int $limit): array
    {
        ...
        $sql .= ' AND NOT EXISTS ('
            . '   SELECT 1 FROM mediamarkt_offer_change_queue active_queue'
        ...
        $cursorKey = 'mediamarkt_warehouse_cursor_v1_' . md5($operation . '|' . $accountScope);
     ...
     * of those trigger the try/catch in MediaMarktService::processQueue(), so without this the row
    ...
    public function reclaimStaleProcessing(int $staleAfterSeconds = 300, int $maxAttempts = 5): array
    {
        ...
        $errored = $this->database->query(
            'UPDATE mediamarkt_offer_change_queue'
    ...
    public function fetchQueueBatch(int $limit = 100, ?int $accountId = null): array
    {
        ...
        $sql = 'SELECT queue.*, offers.shop_sku, offers.product_sku, offers.offer_id, offers.product_title'
            . ' FROM mediamarkt_offer_change_queue queue'
            . ' INNER JOIN mediamarkt_offers offers ON offers.id = queue.offer_row_id'
    ...
    public function markQueueWaiting(int $queueId, string $remoteImportId, string $remoteImportType, int $delaySeconds = 10, array $logPayload = array()): void
    {
        ...
        if ($logPayload !== array()) {
            $current = $this->database->fetchColumn(
                'SELECT payload_json FROM mediamarkt_offer_change_queue WHERE id = :id LIMIT 1',
                array('id' => $queueId)
            );
        ...
        $data = array(
            'status' => 'retry',
            'error_message' => 'Import MediaMarkt przyjety; oczekiwanie na potwierdzenie.',
            'remote_import_id' => $remoteImportId,
            'remote_import_type' => $remoteImportType,
    ...
    public function recentQueueEntries(int $limit = 50): array
    {
        $rows = $this->database->fetchAll(
            ...
            . ' FROM mediamarkt_offer_change_queue queue'
            . ' INNER JOIN mediamarkt_offers offers ON offers.id = queue.offer_row_id'
            . ' INNER JOIN mediamarkt_accounts accounts ON accounts.id = queue.account_id'
    ...
    {
        $row = $this->database->fetch('SELECT attempts FROM mediamarkt_offer_change_queue WHERE id = :id LIMIT 1', array('id' => $queueId));
    ...
    {
        $removed = $this->database->delete('mediamarkt_offer_change_queue', '1=1', array());
    ...
    public function clearQueueStatuses(bool $keepPending = true): array
    {
        ...
        $removed = $this->database->delete('mediamarkt_offer_change_queue', $where, array());
        return array('removed' => (int) $removed);
    }
    ...
    public function clearQueueForOffers(array $offerRowIds): int
    {
        ...
        $sql = 'DELETE FROM mediamarkt_offer_change_queue WHERE offer_row_id IN (' . implode(', ', $placeholders) . ')';
    ...
    public function removeOffersByIds(array $offerRowIds): int
    {
        ...
        $sql = 'DELETE FROM mediamarkt_offers WHERE id IN (' . implode(', ', $placeholders) . ')';
        return (int) $this->database->query($sql, $params)->rowCount();
    }
    ...
    public function getCache(string $key)
    {
        $row = $this->database->fetch(
            'SELECT payload FROM mediamarkt_cache WHERE cache_key = :cache_key AND expires_at >= :now LIMIT 1',
    ...
    public function putCache(string $key, array $payload, int $ttl): void
    {
        ...
        $this->database->query(
            'INSERT INTO mediamarkt_cache (cache_key, payload, expires_at) VALUES (:cache_key, :payload, :expires_at) '
    ...
    private function buildOfferWhere(array $filters, array &$params): string
    {
        ...
        if (in_array($queueStatus, array('pending', 'retry', 'error', 'done', 'processing'), true)) {
            $whereParts[] = ' AND EXISTS ('
                . ' SELECT 1 FROM mediamarkt_offer_change_queue queue_latest'
                . ' WHERE queue_latest.offer_row_id = offers.id'
                . '   AND queue_latest.id = ('
                . '     SELECT MAX(queue_max.id)'
                . '     FROM mediamarkt_offer_change_queue queue_max'
        ...
        if ($errorQuery !== '') {
            $whereParts[] = ' AND EXISTS ('
                . ' SELECT 1 FROM mediamarkt_offer_change_queue queue_latest_error'
                . ' WHERE queue_latest_error.offer_row_id = offers.id'
                . '   AND queue_latest_error.id = ('
                . '     SELECT MAX(queue_max_error.id)'
                . '     FROM mediamarkt_offer_change_queue queue_max_error'
    ...
    private function buildOfferSort(string $sortBy, string $sortDir): string
    {
        ...
        $map = array(
            ...
            'queue_status' => '(SELECT queue_latest.status FROM mediamarkt_offer_change_queue queue_latest WHERE queue_latest.offer_row_id = offers.id ORDER BY queue_latest.id DESC LIMIT 1)',
        );
