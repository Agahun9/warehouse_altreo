...
<div class="market-params market-params--mediamarkt"
  ...
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
    <div>
      ...
      <div class="small text-muted">
        ...
        {if !empty($mediamarkt_parameters_meta.component_category)} | Komponenty: <strong>{$mediamarkt_parameters_meta.component_category|escape:'html'}</strong>{/if}
      </div>
    </div>
    <div class="small text-muted">{if $mediamarktParamsList}{count($mediamarktParamsList)}{else}0{/if} parametrów</div>
  </div>

  {if $mediamarkt_parameters_error|default:''}
    <div class="alert alert-warning py-2 mb-3">{$mediamarkt_parameters_error|escape:'html'}</div>
  {/if}
  ...
  {if $mediamarktParamsList}
    ...
    {if $mediamarkt_parameters_meta.used_count|default:0 > 0 && $mediamarkt_parameters_meta.unused_count|default:0 > 0}
      <div class="alert alert-light border d-flex justify-content-between align-items-center gap-2 py-2">
        <span>Widoczne: <strong>{$mediamarkt_parameters_meta.used_count}</strong> parametrów używanych w istniejących komponentach.</span>
        <button type="button" class="btn btn-sm btn-outline-secondary js-toggle-unused-params" data-hidden-count="{$mediamarkt_parameters_meta.unused_count}">
          Pokaż pozostałe ({$mediamarkt_parameters_meta.unused_count})
        </button>
      </div>
    {/if}
        ...
        <div class="col-12 col-xl-6 js-mediamarkt-param-card{if !$param.is_used && $mediamarkt_parameters_meta.used_count|default:0 > 0} d-none{/if}"
          ...
          <div class="border rounded-3 p-3 h-100 bg-white">
            ...
            {if $param.option_lookup}
              <div class="mediamarkt-option-lookup"
                   data-attribute-id="{$param.id|escape:'html'}"
                   data-multiple="{if $param.multiple}1{else}0{/if}">
                <div class="input-group">
                  <span class="input-group-text"><i class="bi bi-search"></i></span>
                  <input type="search"
                         class="form-control js-mediamarkt-option-search"
                         placeholder="Szukaj dostępnych wartości dla: {$param.name|escape:'html'}"
                         autocomplete="off">
                  <button type="button" class="btn btn-outline-secondary js-mediamarkt-option-clear">Wyczyść</button>
                </div>
                ...
                <div class="list-group mt-2 js-mediamarkt-option-results d-none"></div>
                <div class="mt-2 js-mediamarkt-option-selection">
                  {if $param.multiple}
                    {if $storedValue neq ''}
  ...
  {else}
    <div class="alert alert-light border">Brak parametrów MediaMarkt do wyświetlenia.</div>
  {/if}
  ...
  (function(){
    ...
    mediamarktRoot.querySelectorAll('.mediamarkt-option-lookup').forEach(function (lookup, lookupIndex) {
      ...
      clear.addEventListener('click', function () {
        ...
        var selection = lookup.querySelector('.js-mediamarkt-option-selection');
        if (lookup.getAttribute('data-multiple') === '1') {
          selection.querySelectorAll('.mediamarkt-selected-option').forEach(function (badge) { badge.remove(); });
        } else {
          var valueInput = selection.querySelector('.js-mediamarkt-option-value');
          var notice = selection.querySelector('.js-mediamarkt-single-selection');
      ...
      lookup.addEventListener('click', function (event) {
        ...
        if (remove) remove.closest('.mediamarkt-selected-option').remove();
      });

      var parameterCard = lookup.closest('.js-mediamarkt-param-card');
    ...
    container.addEventListener('click', function (event) {
      if (event.target && event.target.classList.contains('remove-mediamarkt-param')) {
        var row = event.target.closest('.mediamarkt-param-row');
        if (row) {
          row.remove();
        }
      }
