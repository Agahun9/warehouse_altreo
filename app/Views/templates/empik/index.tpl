<main class="app-main">
    ...
    <div class="container-fluid">
      ...
      <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <div class="d-flex flex-wrap gap-2">
            ...
            <a href="{$baseUrl}?controller=empik&action=sync" class="btn btn-sm btn-outline-primary">Synchronizuj aktywne konto</a>
            <a href="{$baseUrl}?controller=empik&action=processqueue" class="btn btn-sm btn-outline-success">Uruchom worker kolejki</a>
          </div>
        </div>
        <div class="card-body">
          <div class="alert alert-light border small text-secondary">
            Empik dziala przez Mirakl Seller API. Konfiguracja kont jest dostepna tylko w <a href="{$baseUrl}?controller=administration&action=automation" class="alert-link">Administracji</a>. W tym widoku worker korzysta z dedykowanych importow Mirakl dla ceny i stanow oraz z aktualizacji ofert dla aktywacji, wznawiania i zmian opisu.
          </div>
          ...
          <div class="row g-3 mb-3">
            <div class="col-md-3">
              <div class="border rounded p-3 h-100">
                ...
                <div class="fs-4 fw-semibold text-warning"><a class="text-warning text-decoration-none" href="{$baseUrl}?controller=empik&action=index&queue_status=pending">{$queueStats.pending}</a></div>
              </div>
            </div>
            <div class="col-md-3">
              <div class="border rounded p-3 h-100">
                <div class="small text-secondary">Ponow</div>
                <div class="fs-4 fw-semibold text-warning"><a class="text-warning text-decoration-none" href="{$baseUrl}?controller=empik&action=index&queue_status=retry">{$queueStats.retry}</a></div>
              </div>
            </div>
            <div class="col-md-3">
              <div class="border rounded p-3 h-100">
                <div class="small text-secondary">Gotowe</div>
                <div class="fs-4 fw-semibold text-success"><a class="text-success text-decoration-none" href="{$baseUrl}?controller=empik&action=index&queue_status=done">{$queueStats.done}</a></div>
              </div>
            </div>
            <div class="col-md-3">
              <div class="border rounded p-3 h-100">
                <div class="small text-secondary">Blad</div>
                <div class="fs-4 fw-semibold text-danger"><a class="text-danger text-decoration-none" href="{$baseUrl}?controller=empik&action=index&queue_status=error">{$queueStats.error}</a></div>
              </div>
            </div>
          </div>

          <div class="d-flex gap-2">
            <form method="post" action="{$baseUrl}?controller=empik&action=clearqueue">
      ...
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#empikBulkModal">Akcje masowe</button>
        </div>

        <div class="card-body border-bottom py-2">
          <div class="d-flex flex-wrap gap-2 align-items-center">
            <button type="button" class="btn btn-sm btn-outline-secondary" id="empik-select-page">Zaznacz strone</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" id="empik-clear-page">Odznacz strone</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" id="empik-invert-page">Odwroc zaznaczenie</button>
          </div>
        </div>
            ...
            <table class="table table-sm table-striped table-hover table-bordered align-middle mb-0">
              <thead class="table-light">
                <tr>
                  ...
                  <th><a href="{$sortUrls.quantity|escape}" class="text-decoration-none text-dark">Empik</a></th>
                ...
                {foreach $offers as $offer}
                  ...
                  <tr>
                    <td class="text-center">
                      <input class="form-check-input empik-offer-checkbox" type="checkbox" value="{$offer.id|escape}">
                    </td>
        ...
        <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
          <div class="btn-group">
            ...
            <a class="btn btn-sm btn-outline-secondary{if $page <= 1} disabled{/if}" href="{$baseUrl}?controller=empik&action=index&page=1&per_page={$perPage}&sort_by={$sortBy|escape:'url'}&sort_dir={$sortDir|escape:'url'}&account_id={$filters.account_id|escape:'url'}&q={$filters.q|escape:'url'}&sku={$filters.sku|escape:'url'}&state={$filters.state|escape:'url'}&active={$filters.active|escape:'url'}&queue_status={$filters.queue_status|escape:'url'}&error_query={$filters.error_query|escape:'url'}&linked={$filters.linked|escape:'url'}&warehouse_quantity_from={$filters.warehouse_quantity_from|escape:'url'}&warehouse_quantity_to={$filters.warehouse_quantity_to|escape:'url'}">Pierwsza</a>
            <a class="btn btn-sm btn-outline-secondary{if $page <= 1} disabled{/if}" href="{$baseUrl}?controller=empik&action=index&page={$prevPage}&per_page={$perPage}&sort_by={$sortBy|escape:'url'}&sort_dir={$sortDir|escape:'url'}&account_id={$filters.account_id|escape:'url'}&q={$filters.q|escape:'url'}&sku={$filters.sku|escape:'url'}&state={$filters.state|escape:'url'}&active={$filters.active|escape:'url'}&queue_status={$filters.queue_status|escape:'url'}&error_query={$filters.error_query|escape:'url'}&linked={$filters.linked|escape:'url'}&warehouse_quantity_from={$filters.warehouse_quantity_from|escape:'url'}&warehouse_quantity_to={$filters.warehouse_quantity_to|escape:'url'}">Poprzednia</a>
            {foreach $pageWindow as $pageItem}
              {if $pageItem.type eq 'page'}
                <a class="btn btn-sm {if $pageItem.is_current}btn-primary{else}btn-outline-secondary{/if}" href="{$baseUrl}?controller=empik&action=index&page={$pageItem.value}&per_page={$perPage}&sort_by={$sortBy|escape:'url'}&sort_dir={$sortDir|escape:'url'}&account_id={$filters.account_id|escape:'url'}&q={$filters.q|escape:'url'}&sku={$filters.sku|escape:'url'}&state={$filters.state|escape:'url'}&active={$filters.active|escape:'url'}&queue_status={$filters.queue_status|escape:'url'}&error_query={$filters.error_query|escape:'url'}&linked={$filters.linked|escape:'url'}&warehouse_quantity_from={$filters.warehouse_quantity_from|escape:'url'}&warehouse_quantity_to={$filters.warehouse_quantity_to|escape:'url'}">{$pageItem.value}</a>
              {else}
                <span class="btn btn-sm btn-outline-secondary disabled">...</span>
              {/if}
            {/foreach}
            <a class="btn btn-sm btn-outline-secondary{if $page >= $totalPages} disabled{/if}" href="{$baseUrl}?controller=empik&action=index&page={$nextPage}&per_page={$perPage}&sort_by={$sortBy|escape:'url'}&sort_dir={$sortDir|escape:'url'}&account_id={$filters.account_id|escape:'url'}&q={$filters.q|escape:'url'}&sku={$filters.sku|escape:'url'}&state={$filters.state|escape:'url'}&active={$filters.active|escape:'url'}&queue_status={$filters.queue_status|escape:'url'}&error_query={$filters.error_query|escape:'url'}&linked={$filters.linked|escape:'url'}&warehouse_quantity_from={$filters.warehouse_quantity_from|escape:'url'}&warehouse_quantity_to={$filters.warehouse_quantity_to|escape:'url'}">Nastepna</a>
            <a class="btn btn-sm btn-outline-secondary{if $page >= $totalPages} disabled{/if}" href="{$baseUrl}?controller=empik&action=index&page={$totalPages}&per_page={$perPage}&sort_by={$sortBy|escape:'url'}&sort_dir={$sortDir|escape:'url'}&account_id={$filters.account_id|escape:'url'}&q={$filters.q|escape:'url'}&sku={$filters.sku|escape:'url'}&state={$filters.state|escape:'url'}&active={$filters.active|escape:'url'}&queue_status={$filters.queue_status|escape:'url'}&error_query={$filters.error_query|escape:'url'}&linked={$filters.linked|escape:'url'}&warehouse_quantity_from={$filters.warehouse_quantity_from|escape:'url'}&warehouse_quantity_to={$filters.warehouse_quantity_to|escape:'url'}">Ostatnia</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="modal fade" id="empikBulkModal" tabindex="-1" aria-labelledby="empikBulkModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="empikBulkModalLabel">Akcje masowe Empik</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Zamknij"></button>
        </div>
        <form method="post" action="{$baseUrl}?controller=empik&action=queue" id="empik-bulk-form">
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <div class="form-check">
                <input class="form-check-input" type="radio" name="selection_scope" id="empik_selection_filtered" value="filtered">
                <label class="form-check-label" for="empik_selection_filtered">Wszystkie z aktualnego filtrowania</label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="selection_scope" id="empik_selection_selected" value="selected" checked>
                <label class="form-check-label" for="empik_selection_selected">Tylko zaznaczone na liscie</label>
              </div>
            </div>

            <div class="mb-3">
              <label class="form-label">Operacja</label>
              <select class="form-select" name="operation" id="empik-operation" required>
              ...
              <div class="form-text">W Empiku przez Mirakl podmieniamy opisy, ceny i stany. Tytul oferty nie jest tutaj wystawiony jako pole importu oferty.</div>
            </div>

            <div class="mb-3 empik-bulk-field" data-ops="set_price,set_description">
              <label class="form-label">Wartosc</label>
              <input type="text" class="form-control" name="value" placeholder="nowa cena lub opis">
            </div>

            <div class="mb-3 empik-bulk-field d-none" data-ops="set_leadtime">
            ...
            <div class="mb-3 empik-bulk-field d-none" data-ops="replace_description">
  ...
  <script>
    (function () {
      var checkboxesSelector = '.empik-offer-checkbox';
      var form = document.getElementById('empik-bulk-form');
      var operation = document.getElementById('empik-operation');
      ...
      function updateBulkFields() {
        ...
        Array.prototype.slice.call(document.querySelectorAll('.empik-bulk-field')).forEach(function (node) {
      ...
      var selectPage = document.getElementById('empik-select-page');
      var clearPage = document.getElementById('empik-clear-page');
      var invertPage = document.getElementById('empik-invert-page');
