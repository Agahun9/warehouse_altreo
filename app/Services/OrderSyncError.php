...
final class OrderSyncError
{
    ...
    public static function describe(\Throwable $error): array
    {
        ...
        if ($status===401 || stripos($text,'refresh token')!==false || stripos($text,'invalid_grant')!==false) {
            ...
            $code='MARKETPLACE_REQUEST'; $message='Marketplace odrzucił parametry pobierania zamówień (HTTP '.$status.'). Kursor pozostał bez zmian.';
        } elseif ($status>=500) {
    ...
    public static function log(\Throwable $error,array $context=[]): array
    {
        ...
        $record=$diagnostic+['exception'=>get_class($error),'file'=>basename($error->getFile()),'line'=>$error->getLine(),'context'=>$context];
