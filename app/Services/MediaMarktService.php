...
class MediaMarktService
{
    ...
    /** @var MediaMarktStorageRepository */
    private $storage;
    ...
    {
        $app = Config::get('app');
        $this->config = isset($app['mediamarkt']) && is_array($app['mediamarkt']) ? $app['mediamarkt'] : array();
        $this->storage = new MediaMarktStorageRepository(Database::instance());
    ...
    public function saveAccount(array $input, ?int $accountId = null): array
    {
        ...
        if ($accountId !== null && !$existing) {
            throw new RuntimeException('Nie znaleziono konta MediaMarkt do edycji.');
        }
        ...
        if ($name === '' || $apiUrl === '' || $apiKey === '') {
            throw new RuntimeException('Uzupelnij nazwe konta, adres API i API key MediaMarkt.');
        }

        if (filter_var($apiUrl, FILTER_VALIDATE_URL) === false) {
            throw new RuntimeException('Adres API MediaMarkt musi byc poprawnym URL.');
        }
    ...
    public function enqueueOfferChanges(array $filters, string $operation, array $payload = array(), array $selectedOfferIds = array()): array
    {
        ...
        if ($operation === '') {
            throw new RuntimeException('Wybierz poprawna operacje dla MediaMarkta.');
        }
    ...
    /**
     * Sends all selected price changes in one Mirakl pricing import per MediaMarkt account.
    ...
    public function submitPriceUpdatesBatch(array $updates): array
    {
        ...
        foreach ($updates as $update) {
            ...
            if ($accountId <= 0 || $shopSku === '' || $price === null) {
                throw new RuntimeException('Niekompletne dane oferty MediaMarkt do zbiorczej aktualizacji ceny.');
            }

            $offerKey = $accountId . ':' . $shopSku;
            if (isset($groups[$accountId]['offers'][$offerKey])) {
                if ($groups[$accountId]['offers'][$offerKey]['price'] !== $price) {
                    throw new RuntimeException('Oferta MediaMarkt ' . $shopSku . ' otrzymala dwie rozne ceny w tej samej aktualizacji.');
                }
                continue;
            }
            ...
            if (!isset($groups[$accountId])) {
                ...
                if (!is_array($account) || $account === array()) {
                    throw new RuntimeException('Nie znaleziono konta MediaMarkt dla zbiorczej aktualizacji ceny.');
                }
        ...
        foreach ($groups as $accountId => $group) {
            ...
            if ($importId === '') {
                throw new RuntimeException('MediaMarkt nie zwrocil identyfikatora zbiorczego importu cen.');
            }
    ...
    public function processQueue(array $options = array()): array
    {
        ...
        foreach ($rows as $row) {
            ...
            try {
                ...
                if (!$offer) {
                    throw new RuntimeException('Nie znaleziono oferty MediaMarkt do przetworzenia.');
                }
                ...
                if ($remoteImportId !== '' && $remoteImportType !== '') {
                    ...
                    if (!$importState['complete']) {
                        ...
                        $logEntry['message'] = 'MediaMarkt nadal przetwarza import; status zostanie sprawdzony ponownie.';
                    ...
                    $logEntry['message'] = 'MediaMarkt potwierdzil aktualizacje oferty.';
                    $summary['log'][] = $logEntry;
                    continue;
                }
                ...
                if ($remoteImportId === '' || $remoteImportType === '') {
                    throw new RuntimeException('MediaMarkt nie zwrocil danych potrzebnych do sprawdzenia importu.');
                }
                ...
                $logEntry['import_id'] = $remoteImportId;
                $logEntry['import_type'] = $remoteImportType;
                $logEntry['message'] = 'Aktualizacja zostala przyjeta przez MediaMarkt; potwierdzenie nastapi w kolejnym przebiegu.';
                $summary['log'][] = $logEntry;
    ...
    public function automationLinks(string $baseUrl): array
    {
        $base = rtrim($baseUrl, '?&');
        $links = array(
            'queue_worker' => $base . '?controller=mediamarkt&action=processqueue&format=json&limit=50',
            'sync_worker' => $base . '?controller=mediamarkt&action=maintenance&format=json&sync=1',
            'maintenance' => $base . '?controller=mediamarkt&action=maintenance&format=json&sync=1',
            'accounts' => array(),
        );
        ...
        foreach ($this->listAccounts() as $account) {
            ...
            $links['accounts'][] = array(
                ...
                'sync' => $base . '?controller=mediamarkt&action=sync&format=json&account=' . $accountSlug,
                'queue_only' => $base . '?controller=mediamarkt&action=processqueue&format=json&account=' . $accountSlug . '&limit=50',
                'maintenance' => $base . '?controller=mediamarkt&action=maintenance&format=json&account=' . $accountSlug . '&sync=1',
            );
        }
    ...
    public function enqueueWarehouseUpdates(string $accountSelector, array $operations, int $limit = 500): array
    {
        ...
        if ($operations === array()) {
            throw new RuntimeException('Wybierz poprawna operacje aktualizacji MediaMarkt z magazynu.');
        }
        ...
        if ($accountSelector !== '') {
            $account = $this->resolveAccount($accountSelector);
            if (!$account) {
                throw new RuntimeException('Nie znaleziono konta MediaMarkt do aktualizacji z magazynu.');
            }
        ...
        foreach ($operations as $operation) {
            ...
            $log[] = array(
                ...
                'message' => 'Oferty dodane do kolejki aktualizacji MediaMarkt.',
            );
        }
    ...
    public function syncAccount(string $accountSelector = '', array $options = array()): array
    {
        $account = $this->resolveAccount($accountSelector);
        if (!$account) {
            throw new RuntimeException('Brak aktywnego konta MediaMarkt do synchronizacji.');
        }
    ...
    public function searchCategories(string $search, bool $forceRefresh = false, ?int $accountId = null): array
    {
        ...
        if (!$account) {
            throw new RuntimeException('Najpierw skonfiguruj aktywne konto MediaMarkt, zeby pobierac kategorie.');
        }

        $cacheKey = 'mediamarkt_hierarchy_search_v1_' . (int) $account['id'] . '_' . md5(mb_strtolower($search, 'UTF-8'));
        if (!$forceRefresh) {
    ...
    {
        $cacheKey = 'mediamarkt_hierarchy_tree_v1_' . (int) $account['id'];
        if (!$forceRefresh) {
            $cached = $this->storage->getCache($cacheKey);
        ...
        if ($rows === array()) {
            throw new RuntimeException('MediaMarkt nie zwrocil drzewa kategorii.');
        }
    ...
    private function normalizeQueuePayload(string $operation, array $payload): array
    {
        switch ($operation) {
            case 'set_price':
                ...
                if ($value === null) {
                    throw new RuntimeException('Podaj poprawna cene dla MediaMarkta.');
                }
            ...
            case 'set_description':
                ...
                if ($value === '') {
                    throw new RuntimeException('Wpisz opis dla operacji MediaMarkt.');
                }
                return array('value' => $value);
            case 'replace_description':
    ...
    private function executeQueueOperation(array $offer, string $operation, array $payload): array
    {
        switch ($operation) {
            ...
            case 'set_price_from_product':
                if (!isset($offer['warehouse_price_gross']) || $offer['warehouse_price_gross'] === null || $offer['warehouse_price_gross'] === '') {
                    throw new RuntimeException('Oferta MediaMarkt nie ma powiazanego produktu z cena magazynowa.');
                }
            ...
            case 'set_stock_from_product':
                if (!isset($offer['warehouse_quantity']) || $offer['warehouse_quantity'] === null || $offer['warehouse_quantity'] === '') {
                    throw new RuntimeException('Oferta MediaMarkt nie ma powiazanego produktu z iloscia magazynowa.');
                }
            ...
            case 'replace_description':
                ...
                if ($currentDescription === '' || $search === '') {
                    throw new RuntimeException('Brak opisu lub frazy do podmiany w ofercie MediaMarkt.');
                }
                return $this->submitOfferPatch($offer, array(
        ...
        throw new RuntimeException('Nieobslugiwana operacja MediaMarkt: ' . $operation);
    }
    ...
    private function submitPriceImport(array $offer, string $price): array
    {
        ...
        if ($shopSku === '') {
            throw new RuntimeException('Oferta MediaMarkt nie ma shop_sku, wiec nie mozna wyslac ceny.');
        }
        ...
        $importId = trim((string) ($response['import_id'] ?? $response['importId'] ?? ''));
        if ($importId === '') {
            throw new RuntimeException('MediaMarkt nie zwrocil identyfikatora importu cen.');
        }
    ...
    private function submitStockImport(array $offer, int $quantity): array
    {
        $shopSku = trim((string) ($offer['shop_sku'] ?? ''));
        if ($shopSku === '') {
            throw new RuntimeException('Oferta MediaMarkt nie ma shop_sku, wiec nie mozna wyslac stanu.');
        }
        ...
        if ($importId === '') {
            throw new RuntimeException('MediaMarkt nie zwrocil identyfikatora importu stanow.');
        }
    ...
    private function submitOfferPatch(array $offer, array $patch): array
    {
        ...
        if (!is_array($payload) || $payload === array()) {
            throw new RuntimeException('Brak kompletnego payloadu oferty MediaMarkt do aktualizacji.');
        }
        ...
        $importId = trim((string) ($response['import_id'] ?? $response['importId'] ?? ''));
        if ($importId === '') {
            throw new RuntimeException('MediaMarkt nie zwrocil identyfikatora importu ofert.');
        }
    ...
    private function requestMultipartImport(array $accountOrOffer, string $path, string $csv): array
    {
        ...
        if ($tmpFile === false) {
            throw new RuntimeException('Nie udalo sie przygotowac pliku importu MediaMarkt.');
        }

        file_put_contents($tmpFile, $csv);

        try {
            $file = new \CURLFile($tmpFile, 'text/csv', 'mediamarkt-import.csv');
    ...
    private function inspectImport(array $offer, string $type, string $importId): array
    {
        ...
        if ($state === 'FAILED') {
            ...
            throw new RuntimeException($error !== '' ? $error : ('Import MediaMarkt zakonczyl sie statusem FAILED (' . $type . ').'));
        }
    ...
    private function fetchImportStatus(array $offer, string $type, string $importId): array
    {
        ...
        throw new RuntimeException('Nieznany typ importu MediaMarkt: ' . $type);
    }
    ...
    ): array
    {
        ...
        $ch = curl_init($url);
        if ($ch === false) {
            throw new RuntimeException('Nie udalo sie zainicjalizowac polaczenia z MediaMarkt API.');
        }
        ...
        $headers = array(
            ...
            'User-Agent: ALTREO-MediaMarkt-Mirakl/1.0',
        );
        ...
        if (!is_string($raw)) {
            throw new RuntimeException('MediaMarkt API nie zwrocilo odpowiedzi.');
        }

        if ($httpCode === 429) {
            $retryAfter = $this->parseRetryAfterSeconds($responseHeaders);
            $retryAfter = $this->rememberApiCooldown($account, $retryAfter);
            throw new MediaMarktRateLimitException(
                'MediaMarkt API zwrocilo HTTP 429. Kolejne wywolanie API dla tego konta zostalo wstrzymane do '
        ...
        if (!is_array($decoded)) {
            throw new RuntimeException('MediaMarkt API zwrocilo niepoprawny JSON.');
        }
    ...
    private function requestRaw(array $account, string $method, string $path, array $query = array()): string
    {
        ...
        if ($httpCode < 200 || $httpCode >= 300) {
            throw new RuntimeException('MediaMarkt API zwrocilo blad HTTP ' . $httpCode . ($curlError !== '' ? ': ' . $curlError : '.'));
        }
    ...
    private function enforceApiCooldown(array $account): void
    {
        ...
        $cached = $this->storage->getCache('mediamarkt:api-rate-limit:account:' . $accountId);
        ...
        throw new MediaMarktRateLimitException(
            'MediaMarkt API pozostaje w okresie ochronnym po HTTP 429 do ' . date(DATE_ATOM, $retryAt) . '.',
            $remaining
        );
    }
    ...
    private function rememberApiCooldown(array $account, ?int $retryAfterSeconds): int
    {
        ...
        if ($accountId > 0) {
            $this->storage->putCache(
                'mediamarkt:api-rate-limit:account:' . $accountId,
    ...
    {
        return (string) $this->configValue('default_locale', 'de_DE');
    }
    ...
    {
        $cacheKey = 'mediamarkt:cache-cleanup-throttle:v1';
        $cached = $this->storage->getCache($cacheKey);
