...
class Database
{
    ...
    /** Tylko dla testów: wstrzykuje połączenie (np. SQLite w pamięci) jako instancję globalną. */
    public static function useInstance(self $database): void
    {
    ...
    {
        // Izolacja firm: om_* / print_* -> t{id}_om_* (patrz App\Core\Tenant).
        $sql = Tenant::rewrite($sql);
    ...
    public function acquireAdvisoryLock(string $name): bool
    {
        ...
        $result = $this->fetchColumn(
            ...
            array('lock_name' => $this->lockName($name))
        );

        return (int) $result === 1;
    }

    /** Blokady MySQL są globalne dla serwera, więc nazwa zawiera bazę i firmę. */
    private function lockName(string $name): string
    {
        $scope = 'sc:' . (isset($this->config['database']) ? (string) $this->config['database'] : '') . ':'
            . (Tenant::active() ? Tenant::id() : 0) . ':';
        $full = $scope . $name;

        return strlen($full) <= 64 ? $full : 'sc:' . hash('sha256', $full);
    }
