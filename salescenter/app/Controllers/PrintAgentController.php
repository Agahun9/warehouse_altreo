...
final class PrintAgentController extends Controller
{
    ...
    {
        // Agent nie ma sesji: firmę wskazuje prefiks tokenu stanowiska (nagłówek) lub tokenu pobrania PDF.
        $token=$this->bearerToken();
        if ($token==='') { $token=(string)($_GET['token']??''); }
        $tenantId=Tenant::idFromToken($token);
        if ($tenantId===null || !$this->tenantActive($tenantId)) { Tenant::clear(); $this->json(['error'=>'Nieprawidłowy lub wyłączony token stanowiska.'],401); exit; }
    ...
    private function tenantActive(int $tenantId): bool
    {
