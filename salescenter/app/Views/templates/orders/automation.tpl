  ...
  <div class="oa-layout">
    ...
    <aside class="oa-side">
      {if $canWrite}<section class="oa-panel">
        ...
        <form method="post" action="index.php?controller=orders&action=save" class="oa-ship-buttons" onsubmit="return confirm('Utworzyć ręczne automatyzacje „Nadaj: …” dla każdego gabarytu paczki? Nadadzą przesyłkę tylko po kliknięciu przycisku w zamówieniu lub na liście.')"><input type="hidden" name="csrf" value="{$csrf|escape}"><input type="hidden" name="operation" value="rule_shipping_buttons"><input type="hidden" name="tab" value="rules">
          <button class="oa-template"><span class="oa-template-icon"><i class="bi bi-box-seam"></i></span><span><strong>Nadawanie jednym kliknięciem</strong><small>Tworzy od razu przyciski „Nadaj: Mała / Średnia / Duża …” (każdy gabaryt z zakładki Przesyłki). Konto i usługa dobierają się same.</small></span><i class="bi bi-plus-lg"></i></button>
        </form>
