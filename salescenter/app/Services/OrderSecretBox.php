...
final class OrderSecretBox
{
    public static function encrypt(array $secret): string
    {
        ...
        if (trim((string)($config['encryption_key']??''))==='') { throw new RuntimeException('Brak klucza encryption_key w app/Config/app.php potrzebnego do szyfrowania danych.'); }
    ...
    public static function decrypt(string $encoded): array
    {
        ...
        $config=Config::get('app'); $key=hash('sha256',(string)($config['encryption_key']??''),true);
